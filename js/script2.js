...
async function getSongs(folder){
    currFolder=folder;
    let a=await fetch(`http://127.0.0.1:3002/Spotify%20Clone%20WebApp/${folder}/`);
...
const playMusic = (track,pause=false)=>{
    ...
    currentsong.src = `/Spotify Clone WebApp/${currFolder}/`+track+".mpeg";
    if(pause==false){
        currentsong.play();
...
async function displayAlbums(){
    let a=await fetch(`http://127.0.0.1:3002/Spotify%20Clone%20WebApp/songs/`);
    let response = await a.text();
    let div=document.createElement("div");
    div.innerHTML = response;
    ...
    let array=Array.from(anchors);
        ...
        if(e.href.includes("/songs")){   
            ...
            let a=await fetch(`http://127.0.0.1:3002/Spotify%20Clone%20WebApp/songs/${folder}/info.json`);
